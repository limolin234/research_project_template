...
- `$deepseek-context-agent`：独立安装的上下文 skill；按需创建 `deepseek_context.md`，保存思路、尝试、否定路线和其他噪声工作材料，不保存查询历史。
...
`fact.md`；只有实际出现值得跨任务保留的思路或尝试时，才通过独立的
`$deepseek-context-agent` 创建并维护 `deepseek_context.md`。Skill 本身不
随模板复制。
